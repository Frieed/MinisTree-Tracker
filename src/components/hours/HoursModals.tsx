    ...
    <AnimatePresence>
        {isOpen && selectedDay && (
            <>
                ...
                <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="fixed bottom-0 left-0 right-0 bg-white rounded-t-[2.5rem] px-5 pt-4 pb-5 z-[1100] shadow-2xl">
...
export const StudiesModal = ({ isOpen, onClose, currentDate, studies, setStudies, onSave, loading }: StudiesModalProps) => (
    <AnimatePresence>
        {isOpen && (
            <>
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} className="fixed inset-0 bg-nature-brown-dark/40 backdrop-blur-sm z-[1000]" />
                <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="fixed bottom-0 left-0 right-0 bg-white rounded-t-[3rem] p-8 z-[1100] shadow-2xl">
