...
interface DeleteLogConfirmationModalProps {
    isOpen: boolean;
    onClose: () => void;
    onConfirm: () => void;
    logDate: string;
...
export const DeleteLogConfirmationModal = ({
    isOpen,
    onClose,
    onConfirm,
    logDate
}: DeleteLogConfirmationModalProps) => {
                ...
                <>
                    ...
                    <div className="fixed inset-0 z-[4100] flex items-center justify-center p-6 pointer-events-none">
                        ...
                        >
                            ...
                            <div className="absolute top-0 right-0 -mr-8 -mt-8 w-32 h-32 bg-amber-50 rounded-full opacity-50" />
                            <div className="absolute bottom-0 left-0 -ml-8 -mb-8 w-24 h-24 bg-nature-cream/30 rounded-full" />

                            <div className="relative text-center space-y-6">
                                <div className="w-20 h-20 bg-amber-50 rounded-[2rem] flex items-center justify-center mx-auto text-amber-500 shadow-inner">
                                    <AlertTriangle size={40} />
                                </div>

                                <div className="space-y-2">
                                    <h3 className="text-xl font-black text-nature-brown-dark uppercase tracking-tight">Delete History?</h3>
                                    <p className="text-nature-brown text-sm font-medium leading-relaxed px-2">
                                        Are you sure you want to delete the visit record from <span className="font-black text-amber-600">{format(parseISO(logDate), 'MMMM dd, yyyy')}</span>? This action cannot be undone.
                                    </p>
                                </div>
